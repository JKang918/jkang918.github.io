...
2) run OLS of $\hat{u}_t$ on $\hat{u}_{t-1}$ (or if $X$ is not strictly exogenous, regress on $\mathbf{\hat{u}}$ and $X$)\
...
1) run OLS of $y$ on $X$ to get $\mathbf{\hat{u}}$\
2) run OLS of $\hat{u_t}$ on $X$ and $\hat{u}_{t-1}, \; \hat{u}_{t-2}, \; \cdots, \; \hat{u}_{t-p}$ \
