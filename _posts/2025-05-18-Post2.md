...
$$
\mathbb{E}\left[\,\hat{\boldsymbol{\beta}} \mid X\,\right] = \boldsymbol{\beta} \qquad \text{(Under A1, A2, A3)}
$$
...
When A1 through A4 are met, the OLS estimator is BLUE - Best Linear Unbiased Estimator and the error variance estimator $\hat{\sigma}^2$ is also an unbiased estimator. And the covariance of matrix $\hat{\boldsymbol{\beta}}$ can be expressed in a closed form.

$$
\hat{\boldsymbol{\beta}}_{\text{OLS}} \text{ is BLUE (Best Linear Unbiased Estimator)}
$$
...
$$
\mathrm{Var}(\hat{\boldsymbol{\beta}} \mid X) = \sigma^2 (X^{\top} X)^{-1}
$$
...
$$
\mathbf{u} \mid X \sim \mathcal{N}(0,\, \sigma^2 I)
$$
...
With A2 satisfied, we can derive the closed form of $\hat{\boldsymbol{\beta}}_{OLS}$.

The OLS estimator $\hat{\boldsymbol{\beta}}$ minimizes the sum of squared residuals:

$$
\hat{\boldsymbol{\beta}} = \arg \min_\boldsymbol{\beta} \sum_{i=1}^n u_i^2 = \arg \min_\boldsymbol{\beta} \mathbf{u}^{\top} \mathbf{u} = \arg \min_\boldsymbol{\beta} (\mathbf{y} - X\boldsymbol{\beta})^{\top} (\mathbf{y} - X\boldsymbol{\beta})
$$
...
&= \mathbf{y}^{\top} \mathbf{y} - \boldsymbol{\beta}^{\top} X^{\top} \mathbf{y} - \mathbf{y}^{\top} X \boldsymbol{\beta} + \boldsymbol{\beta}^{\top} X^{\top} X \boldsymbol{\beta} \\\\
...
&\frac{\partial}{\partial \boldsymbol{\beta}} (\boldsymbol{\beta}^{\top} X^{\top} \mathbf{y}) = X^{\top} \mathbf{y} \\\\
&\frac{\partial}{\partial \boldsymbol{\beta}} (\boldsymbol{\beta}^{\top} X^{\top} X \boldsymbol{\beta}) = 2 X^{\top} X \boldsymbol{\beta}
\end{aligned}
$$
...
$$
-2 X^{\top} \mathbf{y} + 2 X^{\top} X \hat{\boldsymbol{\beta}} = 0
$$
...
$$
\hat{\boldsymbol{\beta}}_{\text{OLS}} = \arg\min_{\boldsymbol{\beta}} \| \mathbf{y} - \hat{\mathbf{y}} \| 
= \arg\min_{\boldsymbol{\beta}} \| \mathbf{y} - X \boldsymbol{\beta} \|
$$

where $\hat{\mathbf{y}} = \text{Proj}_{\text{col} X} \mathbf{y}$


Here, you can see that OLS attempts to explain $\mathbf{y}$ as much as possible using the columns of $X$ (i.e., the regression parameters). The resulting $\hat{\mathbf{y}}$ represents the best approximation of $\mathbf{y}$ within the column space of $X$. Put differently, $\hat{\mathbf{y}}$ is the projection of $\mathbf{y}$ onto the column space of $X$.

In other words, because $\hat{\mathbf{y}}$ is the projection of $\mathbf{y}$ onto the column space of $X$, the difference $(\mathbf{y} - \hat{\mathbf{y}})$ belongs to the orthogonal space (null space) relative to the columns of $X$.

That is:

$$
(\mathbf{y} - \hat{\mathbf{y}}) \cdot \mathbf{x}_j = 0 \qquad (j = 1, \ldots, k+1)
$$

$$
\implies X^{\top} (\mathbf{y} - \hat{\mathbf{y}}) = 0
$$

$$
\iff X^{\top} (\mathbf{y} - X \hat{\boldsymbol{\beta}}) = 0
$$
...
$$
\begin{align*}
\hat{\boldsymbol{\beta}} &= (X^{\top} X)^{-1} X^{\top} \mathbf{y} \quad \text{(A2)} \\
            &= (X^{\top} X)^{-1} X^{\top} (X\boldsymbol{\beta} + \mathbf{u}) \quad \text{(A1)} \\
            &= \boldsymbol{\beta} + (X^{\top} X)^{-1} X^{\top} \mathbf{u} \\
\end{align*}
$$
...
$$
\begin{align*}
\mathbb{E}[\hat{\boldsymbol{\beta}} \mid X]
            &= \mathbb{E}\left[\, \boldsymbol{\beta} + (X^{\top} X)^{-1} X^{\top} \mathbf{u} \mid X \,\right] \\
            &= \boldsymbol{\beta} + (X^{\top} X)^{-1} X^{\top} \mathbb{E}[\mathbf{u} \mid X] \\
            &= \boldsymbol{\beta} + (X^{\top} X)^{-1} X^{\top} \cdot 0 \quad \text{(A3)} \\
            &= \boldsymbol{\beta}
            &\square
\end{align*}
$$
...
$$
\mathrm{Var}\left[\,\hat{\boldsymbol{\beta}} \mid X\,\right] = \sigma^2 (X^{\top} X)^{-1}
$$
...
Plug in the regression model $\mathbf{y} = X\boldsymbol{\beta} + \mathbf{u} \quad \text{(A1)}$:

$$
\begin{align*}
\hat{\boldsymbol{\beta}} &= (X^{\top} X)^{-1} X^{\top} (X\boldsymbol{\beta} + \mathbf{u}) \\
\quad\,\, &= (X^{\top} X)^{-1} X^{\top} X \boldsymbol{\beta} + (X^{\top} X)^{-1} X^{\top} \mathbf{u} \\
\quad\,\, &= \boldsymbol{\beta} + (X^{\top} X)^{-1} X^{\top} \mathbf{u}
\end{align*}
$$
...
&= \mathrm{Var}\left[\,\boldsymbol{\beta} + (X^{\top} X)^{-1} X^{\top} \mathbf{u} \,\middle|\, X \right] \\
...
$$
\begin{align*}
\mathrm{Var}[\hat{\boldsymbol{\beta}} \mid X] &= \mathbb{E}\left[\left(\hat{\boldsymbol{\beta}} - \mathbb{E}[\hat{\boldsymbol{\beta}} \mid X]\right)\left(\hat{\boldsymbol{\beta}} - \mathbb{E}[\hat{\boldsymbol{\beta}} \mid X]\right)^{\top} \mid X \right] \qquad \text{(def. of variance)} \\
                                 &= \mathbb{E}\left[\left(\hat{\boldsymbol{\beta}} -\boldsymbol{\beta} \right)\left(\hat{\boldsymbol{\beta}} - \boldsymbol{\beta} \right)^{\top} \mid X \right] \qquad (\mathbb{E}[\hat{\boldsymbol{\beta}} \mid X] = \boldsymbol{\beta} \; \text{by A3})\\
                                 &= \mathbb{E}\left[ (X^{\top} X)^{-1} X^{\top} \mathbf{u} \left( (X^{\top} X)^{-1} X^{\top} \mathbf{u} \right)^{\top} \mid X \right] \qquad (\hat{\boldsymbol{\beta}} = \boldsymbol{\beta} + (X^{\top} X)^{-1} X^{\top} \mathbf{u} \; \text{by A2}) \\
                                 &= \mathbb{E}\left[ (X^{\top} X)^{-1} X^{\top} \mathbf{u} \mathbf{u}^{\top} X (X^{\top} X)^{-1} \mid X \right] \\
                                 &= (X^{\top} X)^{-1} X^{\top}\, \mathbb{E}[ \mathbf{u} \mathbf{u}^{\top} \mid X ]\, X (X^{\top} X)^{-1} \\
                                 &= (X^{\top} X)^{-1} X^{\top} (\sigma^2 I) X (X^{\top} X)^{-1} \qquad (\mathbb{E}[\mathbf{u} \mathbf{u}^{\top} \mid X] = \mathrm{Var}(\mathbf{u} \mid X) = \sigma^2 I \; \text{by A4})\\
                                 &= \sigma^2 (X^{\top} X)^{-1} X^{\top} X (X^{\top} X)^{-1} \\
                                 &= \sigma^2 (X^{\top} X)^{-1}
                                 & \square
\end{align*}
...
$\hat{\boldsymbol{\beta}}_{\text{OLS}} = (X^\top X)^{-1} X^\top \mathbf{y}$  
Let $\tilde{\boldsymbol{\beta}} = A \mathbf{y}$ be **another linear unbiased estimator** (for some matrix $A$).

**Unbiasedness of $\tilde{\boldsymbol{\beta}}$**

$$
\begin{align*}
\mathbb{E}[\tilde{\boldsymbol{\beta}} \mid X] &= \mathbb{E}[A \mathbf{y} \mid X] \\
                                 &= \mathbb{E}[A(X\boldsymbol{\beta} + \mathbf{u}) \mid X] \\
                                 &= \mathbb{E}[AX\boldsymbol{\beta} \mid X] + \mathbb{E}[A\mathbf{u} \mid X] \\
                                 &= \mathbb{E}[AX\boldsymbol{\beta} \mid X] \\
                                 &= AX\boldsymbol{\beta} = \boldsymbol{\beta}\\
...
$$
\begin{align*}
\mathrm{Var}[\tilde{\boldsymbol{\beta}} \mid X] &= \mathrm{Var}[A\mathbf{y} \mid X]\\
                                   &= \mathrm{Var}[A(X\boldsymbol{\beta} + \mathbf{u}) \mid X]\\
                                   &= \mathrm{Var}[(AX\boldsymbol{\beta} + A \mathbf{u}) \mid X]\\
                                   &= \mathrm{Var}[(I_{k+1}\boldsymbol{\beta} + A \mathbf{u}) \mid X] \qquad \text{(from above result)}\\
                                   &= \mathrm{Var}[(\boldsymbol{\beta} + A \mathbf{u}) \mid X]  \\
                                   &= \mathrm{Var}[A \mathbf{u} \mid X] \\
                                   &= A\mathrm{Var}[\mathbf{u} \mid X]A^\top \\
...
$$
\mathrm{Var}[\hat{\boldsymbol{\beta}} \mid X] = \sigma^2 (X^\top X)^{-1} \leq \mathrm{Var}[\tilde{\boldsymbol{\beta}} \mid X] = \sigma^2 A A^\top \iff A A^\top - (X^\top X)^{-1} \quad \text{is positive semi-definite.}
$$
...
OLS is the process of orthogonally projecting $\mathbf{y}$ onto the column space of $X$ and the resultant projected vector is $\hat{\mathbf{y}}$, which is:

$$
\hat{\mathbf{y}} = X\hat{\boldsymbol{\beta}} =\underbrace{X(X^\top X)^{-1} X^\top}_{\text{proj matrix onto the col space}} \mathbf{y} = P \mathbf{y}
$$

In this sense, $P$ is the projection matrix onto the column space of $X$.

How about $\hat{\mathbf{y}} - \mathbf{y}$? Obviously, this belongs to the orthogonal complement of column space of $X$.

$$
\hat{\mathbf{y}} - \mathbf{y} = \mathbf{y} - X(X^\top X)^{-1} X^\top \mathbf{y} = \underbrace{(I -P)}_{\text{proj matrix onto the orthogonal compl}} \mathbf{y} = M \mathbf{y}
$$
...
- $M X = 0$
- $P = X$, $P^2 = P$, $P^\top = P$
- $\mathbf{y} = P \mathbf{y} + M \mathbf{y}$ (fitted + residual decomposition)
...
where
$$
\hat{\sigma}^2 = \frac{\hat{\mathbf{u}}^\top \hat{\mathbf{u}}}{n - (k + 1)}
$$

**proof**

$\hat{\mathbf{u}} = M y \qquad (M : \text{residual maker})$

where $M = I - X (X^\top X)^{-1} X^\top$

$\Rightarrow \hat{\mathbf{u}} = M u$

($My = M(X\beta + \mathbf{u}) = M X \beta + M \mathbf{u} = 0 + M \mathbf{u} = M \mathbf{u}$)

Then,

$\hat{\mathbf{u}}^\top \hat{\mathbf{u}} = (M \mathbf{u})^\top (M \mathbf{u})$

$= \mathbf{u}^\top M^\top M \mathbf{u} \quad \text{(Symmetric)}$

$= \mathbf{u}^\top M M u$

$= \mathbf{u}^\top M \mathbf{u} \qquad \text{(Idempotent)} \rightarrow \text{quadratic form}$

With this, compute the expectation:

$$
\begin{align*}
\mathbb{E}\left[ \hat{\mathbf{u}}^\top \hat{\mathbf{u}} \mid X \right]
&= \mathbb{E}\left[ \mathbf{u}^\top M \mathbf{u} \mid X \right]\\
&= \mathbb{E}\left[ \operatorname{tr}(\mathbf{u}^\top M \mathbf{u}) \mid X \right]\\
&= \mathbb{E}\left[ \operatorname{tr}(M \mathbf{u} \mathbf{u}^\top) \mid X \right]\\
&= \operatorname{tr}\left( M \mathbb{E}[\mathbf{u} \mathbf{u}^\top \mid X] \right)\\
...
$$
\hat{\boldsymbol{\beta}} \mid X \sim \mathcal{N}(\boldsymbol{\beta},\, \sigma^2 (X^\top X)^{-1})
$$
...
$$
\mathrm{se}(\hat{\beta}_j) = \sqrt{\hat{\sigma}^2} = \sqrt{ \frac{ \hat{\mathbf{u}}^\top \hat{\mathbf{u}} }{ n - (k+1) } }
$$

$$
\frac{ \mathrm{se}(\hat{\beta}_j) }{ \mathrm{sd}(\hat{\beta}_j) }
= \sqrt{ \frac{ \hat{\mathbf{u}}^\top \hat{\mathbf{u}} / (n-(k+1)) }{ \sigma^2 } } = \sqrt{ \frac{ \hat{\mathbf{u}}^\top \hat{\mathbf{u}} /  \sigma^2}{ (n-(k+1)) } }
= \sqrt{ \frac{W}{(n-(k+1))} }
$$
...
Define:
$$
W = \frac{ \hat{\mathbf{u}}^\top \hat{\mathbf{u}} }{ \sigma^2 }
$$
where
$$
\hat{\mathbf{u}}^\top \hat{\mathbf{u}} = \mathbf{u}^\top M \mathbf{u}
$$
 (refer to Theorem 4.) and $M = I - P$ is the residual maker matrix.

Thus,
$$
W = \frac{\mathbf{u}^\top M \mathbf{u}}{\sigma^2}
$$
...
- $Z$ is a function of $\hat{\boldsymbol{\beta}}$

Let $A = (X^\top X)^{-1} X^\top$ and $B = M$.

If $\mathbf{u} \sim \mathcal{N}(0, \sigma^2 I_n)$,  
then $A\mathbf{u}$ and $B\mathbf{u}$ are independent if $AB^\top = 0$. (But why? Also, you can either accept this as a statistical fact or refer to [Technical Note 4](#technical-note-4))
...
Then:
$$
\mathbf{y}^\top A \mathbf{y} \sim \chi^2_q
$$
...
Given that
$$
Av = \lambda \mathbf{v} \qquad (\mathbf{v}: \text{ eigenvector,} \; \lambda: \text{ eigenvalue})
$$

the following two are identical:

$$
A^2v = A(A\mathbf{v}) = A(\lambda \mathbf{v}) = \lambda (A\mathbf{v}) = \lambda^2\mathbf{v}
$$
...
Now let's proceed to $\mathbf{y}^\top A \mathbf{y} \sim \chi^2_q$.

$$
\begin{align*}
\mathbf{y}^\top A \mathbf{y} &= \mathbf{y}^\top P\Lambda P^\top \mathbf{y} \qquad &(A = P \Lambda P^\top \text{ orthogonal diagonalization})\\
           &= \mathbb{z}^\top \Lambda \mathbb{z} \qquad &(\mathbb{z} = P^\top \mathbf{y}) \\
...
Lastly, let's check why $\mathbb{z} \sim \mathcal{N}(\mathbb{0},\, I)$ given $\mathbf{y} \sim \mathcal{N}(\mathbb{0}, I)$.

$$
\begin{align*}
\mathbb{E}\left[\mathbb{z}\right] = \mathbb{E}\left[P^\top \mathbf{y}\right] = P^\top \mathbb{E}\left[\mathbf{y}\right] = P^\top \mathbb{E}\left[\mathbf{y}\right] = \mathbb{0}
\end{align*}
$$
...
Under the normality condition, $A\mathbf{u}, \; B\mathbf{u}: \text{ independent } \iff AB^\top = 0$.
...
$$
\begin{align*}
Cov(A\mathbf{u}, B\mathbf{u}) &= \mathbb{E}\left[(A\mathbf{u})(B\mathbf{u})^\top \right] \\
            &= \mathbb{E}\left[A\mathbf{u}\mathbf{u}^\top B^\top \right]  \\
            &= A \mathbb{E}\left[\mathbf{u}\mathbf{u}^\top  \right]  B^\top \\
...
$$
F = \frac{(R\hat{\boldsymbol{\beta}} - \mathbf{r})^{\top} \left[ R(X^{\top}X)^{-1}R^{\top} \right]^{-1} (R\hat{\boldsymbol{\beta}} - \mathbf{r}) / q}{\hat{\sigma}^2}
\sim F_{q,\, n-k-1}
$$
...
**Example of $R\boldsymbol{\beta} = \mathbf{r}$**
...
- Under $H_0: R\boldsymbol{\beta} = \mathbf{r}$,
- $R\hat{\boldsymbol{\beta}}$ is a $q \times 1$ vector:

$$
R\hat{\boldsymbol{\beta}} \sim \mathcal{N}(R\boldsymbol{\beta}, \sigma^2 R(X^{\top}X)^{-1}R^{\top})
$$

- Thus, (by statistical fact 3 below)

$$
(R\hat{\boldsymbol{\beta}} - \mathbf{r})^{\top} \left[ \sigma^2 R(X^{\top}X)^{-1}R^{\top} \right]^{-1} (R\hat{\boldsymbol{\beta}} - \mathbf{r}) \sim \chi^2_q
$$

- The above can be rearranged as follows.

$$
\frac{1}{\hat{\sigma}^2} (R\hat{\boldsymbol{\beta}} - \mathbf{r})^{\top} [R(X^{\top}X)^{-1}R^{\top}]^{-1} (R\hat{\boldsymbol{\beta}} - \mathbf{r}) \sim \chi^2_q
$$
...
$$
\frac{(R\hat{\boldsymbol{\beta}} - \mathbf{r})^{\top} [R(X^{\top}X)^{-1}R^{\top}]^{-1} (R\hat{\boldsymbol{\beta}} - \mathbf{r}) / q}{\hat{\sigma}^2}
\sim F_{q, n-k-1}
$$
